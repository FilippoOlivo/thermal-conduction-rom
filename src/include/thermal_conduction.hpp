...
public:
  ...
  ThermalConduction(
      std::vector<double> regions, std::vector<double> conductivities,
      std::vector<double> boundary_temperatures = {0.0, 0.0, 500.0, -1.0},
      unsigned int axis = 0, std::string parameter_file = "params.prm");

  void compute_stiffness_matrix();

  SparseMatrix<double> &get_stiffness_matrix();

  std::pair<Vector<double>, SparseMatrix<double>>
  get_affine_components(unsigned int i);

  unsigned int get_num_regions();
...
private:
  ...
  void assemble_system(bool apply_bc_to_solution = false);

  void apply_boundary_conditions(bool apply_to_solution = false);

  void assemble_system_matrix(FEValues<dim> &fe_values,
                              FullMatrix<double> &cell_matrix, int region);

  void assemble_system_rhs(
      FEFaceValues<dim> &fe_face_values, Vector<double> &cell_rhs,
      const typename DoFHandler<dim>::active_cell_iterator &cell);

  int get_point_region(const Point<2> &point);

  void apply_diagonal_boundary_conditions(
      std::map<types::global_dof_index, double> &boundary_values,
      bool apply_to_solution = false);
